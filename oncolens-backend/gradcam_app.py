...
from server import app, encoder, lock, model, processor, v2_classifier, v2_threshold

scaler = v2_classifier.named_steps["scale"]
classifier = v2_classifier.named_steps["clf"]
...
# Conservative H&E plausibility gate for the hackathon prototype.
# This is intentionally a screening heuristic, not a validated pathology detector.
# It is designed to reject obvious natural photographs before they reach the
# benign/malignant classifier, because a binary classifier will otherwise be
# forced to choose one of those classes for any image at all.
def he_histopathology_gate(image):
    ...
    # Exclude near-white background and near-grey regions when estimating stain coverage.
    tissue = (maximum < 0.985) & (chroma > 0.05)

    # Broad H&E-like colour families. H&E sections usually contain both a
    # blue/purple nuclear component and an eosin-pink tissue component.
    purple = (
        tissue
        & (r > 0.22)
        & (b > 0.22)
    ...
    stain_fraction = float(np.logical_or(purple, pink).mean())

    # Texture is only a weak supporting signal; the colour/stain requirements
    # below are the main rejection criteria.
    gray = 0.299 * r + 0.587 * g + 0.114 * b
    vertical_detail = float(np.abs(np.diff(gray, axis=0)).mean()) if gray.shape[0] > 1 else 0.0
    horizontal_detail = float(np.abs(np.diff(gray, axis=1)).mean()) if gray.shape[1] > 1 else 0.0
    detail = (vertical_detail + horizontal_detail) / 2.0

    # Tuned conservatively for ordinary H&E/BreakHis-style images. This gate
    # should be replaced by a trained histopathology-vs-non-pathology/OOD model
    # before any serious validation or deployment.
    passed = (
        tissue_fraction >= 0.20
        and purple_fraction >= 0.10
        and pink_fraction >= 0.08
        and stain_fraction >= 0.18
        and detail >= 0.008
    )

    # A simple 0-100 UI score; it is not a probability.  The weakest
    # required H&E feature limits the score so ordinary photographs do not
    # look deceptively close to passing.
    score = 100.0 * min(
        1.0,
        tissue_fraction / 0.20,
        purple_fraction / 0.10,
        pink_fraction / 0.08,
        stain_fraction / 0.18,
        detail / 0.008,
    )
    ...
    return {
        "passed": bool(passed),
        "score": round(score, 1),
        "tissueFraction": round(tissue_fraction, 4),
        "purpleFraction": round(purple_fraction, 4),
        "pinkFraction": round(pink_fraction, 4),
        "stainFraction": round(stain_fraction, 4),
        "detail": round(detail, 5),
        "method": "heuristic-he-plausibility-v1",
    }


def require_histopathology(image):
    gate = he_histopathology_gate(image)
    if not gate["passed"]:
        raise HTTPException(
            422,
            "This image does not resemble an H&E histopathology slide. "
            "Upload a breast H&E histopathology image; ordinary photos are not analysed.",
        )
    return gate


def heatmap_rois(heatmap, max_rois=4):
    """Convert the strongest Grad-CAM regions into frontend ROI boxes.

    Coordinates are percentages (0-100), matching the React Roi type:
    x, y, w, h, score. These are attribution regions, not tumor boundaries.
    """
    ...
    threshold = max(0.50, 0.65 * float(values.max()))
    ...
    for y in range(rows):
        for x in range(cols):
            ...
            y0, y1 = min(ys), max(ys)
            x0, x1 = min(xs), max(xs)
            peak = float(max(values[yy, xx] for yy, xx in coords))
            area = len(coords)
            components.append((peak, area, x0, y0, x1, y1))

    components.sort(key=lambda item: (item[0], item[1]), reverse=True)
    rois = []
    for peak, area, x0, y0, x1, y1 in components[:max_rois]:
        # Expand by one patch so very small high-activation islands remain visible.
        x0 = max(0, x0 - 1)
        y0 = max(0, y0 - 1)
        x1 = min(cols - 1, x1 + 1)
        y1 = min(rows - 1, y1 + 1)
        rois.append(
            {
                "x": round(100.0 * x0 / cols, 2),
                "y": round(100.0 * y0 / rows, 2),
                "w": round(100.0 * (x1 - x0 + 1) / cols, 2),
                "h": round(100.0 * (y1 - y0 + 1) / rows, 2),
                "score": round(100.0 * peak, 1),
            }
        )
    return rois
...
def analyze(image):
    with lock, torch.inference_mode(False), torch.enable_grad():
        ...
        try:
            ...
            malignant_index = labels.index("malignant")
            malignant_probability = float(probabilities[malignant_index])
            prediction = "malignant" if malignant_probability >= v2_threshold else "benign"
            best = labels.index(prediction)
    ...
    rois = heatmap_rois(heatmap) if status == "available" else []
    return {
...
def frontend_quality(image: UploadFile = File(...)):
    ...
    he_gate = he_histopathology_gate(decoded)
    metrics = [
        ...
        {
            "label": "H&E histopathology plausibility",
            "value": he_gate["score"],
            "unit": "%",
            "ok": he_gate["passed"],
        },
    ...
    return {
        ...
        "inputGate": he_gate,
        "note": (
            "Research input gate only. It rejects obvious non-H&E photographs before inference; "
            "it is not a diagnostic slide-quality or pathology-validation model."
        ),
    }
...
def frontend_analysis(
    ...
    data = _read_upload(image)
    decoded = decode_image(data)
    require_histopathology(decoded)
    try:
        raw = analyze(decoded)
    except Exception:
...
async def assistant_api(payload: dict):
    ...
    if "her2" in q:
        text = (
            "HER2 is not directly predicted by the current Phikon-v2 model. "
            "For a malignant or suspicious breast histology result, HER2 should "
            "be confirmed using validated IHC and, when appropriate, ISH testing."
        )

    elif "ki-67" in q or "ki67" in q:
        text = (
            "Ki-67 is a proliferation marker and is not quantified by this model. "
            "Its value should be obtained from validated immunohistochemistry."
        )

    elif "heatmap" in q or "grad-cam" in q or "gradcam" in q:
        text = (
            "The Grad-CAM heatmap highlights image regions that contributed most "
            "to the model prediction. It is an explainability aid and does not "
            "represent confirmed tumour boundaries."
        )

    elif "suspicious" in q or "classified" in q:
        text = (
            "The classification is based on Phikon-v2 histopathology image features "
            "processed by the fitted downstream classifier. The result requires "
            "pathologist review and should not be treated as a standalone diagnosis."
        )

    elif "next" in q or "diagnostic" in q:
        text = (
            "Recommended next steps are pathologist review and, when clinically "
            "appropriate, confirmatory ER, PR, HER2 and Ki-67 testing using validated "
            "laboratory methods."
        )

    elif "limitation" in q:
        text = (
            "This research prototype performs image-based benign/malignant "
            "classification and explainability. It does not independently determine "
            "HER2, ER, PR or Ki-67 status and is not a replacement for clinical diagnosis."
        )

    else:
        text = (
            "I can explain the current OncoLens prediction, Grad-CAM heatmap, "
            "recommended confirmatory biomarkers and limitations of this research prototype."
        )
